# 0.5.2-test 安装与实机测试（Phase 1–5 + UI 生命周期与输入修复）

本文件对应安装包 `WOTCTrainerPanel-0.5.2-test`。
...
- **本轮修复尚未实机验证。** 本文中的“预期结果”仍然是设计意图，不是已经取得的实测结果。
...
解压 `WOTCTrainerPanel-0.5.2-test.zip` 后：

```text
WOTCTrainerPanel-0.5.2-test/          <- 把这个目录当作 Mod 目录加入启动器
├─ README.zh-CN.md                    <- 本文件
...
1. 把 `WOTCTrainerPanel-0.5.2-test` 整个目录解压到游戏目录以外的位置。
...
- 官方 WOTC SDK 编译（0.5.2-test）：`Success - 0 error(s), 8 warning(s)`；其中 `WOTCTrainerPanel` 源码 **0 warning**，8 个警告来自原版 SDK 的 DLC 内容。
- 脚本包哈希：见 `PACKAGE.json`。
- **0.5.2-test 没有启动游戏、没有做任何实机操作。编译通过不等于功能正常。** 本轮修复是否生效，要看战术层入口的点击链日志（`clicked` → `requested` → `opened`）和面板生命周期日志。
