...
- **Covert Action**：`XComGameState_CovertAction.uc:1325-1370` `Update` 在 `bStarted && EndDateTime < 当前时间` 时执行 `ApplyRisks` 与 `CompleteCovertAction` 并置 `bCompleted`。本项最终实现与风险取舍见下方 Phase 5b 的更正条目。
- 文本输入沿用工程既有方式：`UIInputDialogue.uc:26` `TInputDialogData` + `Movie.Pres.UIInputDialog`（资源页已在用），物品搜索不新增输入控件。

## Phase 5b — 战略层编码后补充核对（2026-09-25，编译通过后）

- **化身进度不能只减 `Doom`**：`XComGameState_HeadquartersAlien.uc:491` `GetCurrentDoom(true)` = `AlienHQ.Doom` + 所有 `Available` 任务点的 `Doom`，因此只写 `Doom` 无法保证界面总进度下降。本版按任务点 `Doom` 从大到小先扣减（集合与 `GetCurrentDoom` 求和一致），扣完再减 `AlienHQ.Doom`，并用提交后重读 `GetCurrentDoom(true)` 校验，同时把"设施扣了多少、核心扣了多少"回报到结果文本。
- **`XComGameState_HeadquartersAlien.RemoveDoomFromFortress` 不可复用**：它在 `if (MissionState != none)` 判断之前就解引用 `MissionState.Doom`，而 `GetAndAddFortressMission` 可能返回 none，属原版潜在空指针路径。训练器不调用它，改为直接遍历 `Available` 任务点。
- **Covert Action 完成方式更正 Phase 5 结论**：`XComGameState_CovertAction.uc:1296-1370` 的 `ShouldUpdate` 用 `!bCompleted` 判定，`Update` 的顺序是 `ApplyRisks` →（`bAmbushed` 时置 `XComHQ.bWaitingForChosenAmbush`，否则 `CompleteCovertAction`）→ `bCompleted = true`；而 `CompleteCovertAction` 自身不设置 `bCompleted`。若只调 `CompleteCovertAction`，下一 tick 会再次进入 `Update`，触发 `UpdateGameBoard:1910` 的 `\`assert(bUpdated)`。本版因此按原版顺序先写 `bCompleted = true`（并清 `bAmbushed`）再调用 `CompleteCovertAction`：奖励、完成弹窗、`CovertActionCompleted` 事件、抵抗组织活动记录与新手行动补充都走原版；**不调用 `ApplyRisks`**，即不结算行动风险，界面确认框已明确写出这一点。
- **队列类立即完成的原版依据**：`OnProjectCompleted()` 是原版"计时到期"与"UIChooseProject 立即完成试验场项目"共用的例程，且每个子类自行发起总部指令（研究项目走 `eHeadquartersOrderType_ResearchCompleted`）。训练器直接调用它，不自行拆解项目状态、不手写 `BuildQueue`。
- **`bInstant` 不是开关**：`XComGameState_HeadquartersProject` 的 `bInstant` 只在项目**开始**时被读取（并用于显示），对已在运行的项目没有作用，因此不能据此实现"瞬间研究 ON/OFF"。
- **战略层没有可供 mod 挂载的逐 tick 钩子**：`X2DownloadableContentInfo` 与 `X2EventListenerTemplate.RegisterInStrategy` 只提供 `OnGeoscapeEntry` 等入口。`XComCheatManager.uc` 的 `m_bStrategyAllFacilitiesFree` / `m_bStrategyAllFacilitiesInstaBuild`（由 `setstrategyfacilitiesfree` / `setstrategyfacilitiesinstantbuild` 写入）在整个 SDK 的 `.uc` 中**没有任何读取点**，是死开关。在不新增架构的前提下，`docs/REQUEST.zh-CN.txt` 第五节的 "Instant … ON/OFF" 只能做成一次性的"立即完成"操作，界面按此命名，不伪装成开关。
