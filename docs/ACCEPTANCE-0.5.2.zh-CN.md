...
适用安装包：`WOTCTrainerPanel-v0.9.0-beta`（安装见 `docs/INSTALL-AND-TEST.zh-CN.md`）。

> **0.9.0-beta 说明：** 相对 0.5.2-test 只改了 4 处界面标签的文案来源（硬编码英文 → 本地化键），没有改任何功能逻辑、数值写入、安全检查或禁用状态。因此本清单**整份继续有效**，直接用 0.9.0-beta 的包跑即可。本清单文件名保留 `0.5.2` 是为了不改动已经发出去的编号。
