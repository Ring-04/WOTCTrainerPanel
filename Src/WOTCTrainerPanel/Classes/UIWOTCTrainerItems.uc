...
var WOTCTrainerButton Rows[12], FilterButton;
...
simulated function OnInit()
{
	...
	PageText = AddText(Root, 'Page', "", 30, 85, 320, 40);
	AddButton(Root, 'Previous', "<", 360, 85, 90, 802);
	AddButton(Root, 'Next', ">", 458, 85, 90, 803);
	FilterButton = AddButton(Root, 'Filter', "", 560, 85, 790, 804);
	...
	AddButton(Root, 'SetQuantity', class'WOTCTrainerText'.default.SetQuantity, 610, 400, 360, 300);
	AddButton(Root, 'Grant', class'WOTCTrainerText'.default.GrantItem, 990, 400, 360, 400);
	...
	Quantity = 1;
	Cats = class'WOTCTrainerItems'.static.Categories();
	RefreshCatalogue();
}
...
{
	Filtered = class'WOTCTrainerItems'.static.Items(CategoryName(), RowLabels, Excluded);
...
simulated function RefreshRows()
{
	...
	NoticeText.SetText(class'WOTCTrainerText'.default.ItemHelp $ "<br><br>"
		$ class'WOTCTrainerText'.default.ItemExcluded @ Excluded $ " " $ class'WOTCTrainerText'.default.ItemExcludedWhy);
}
...
simulated function OnAction(UIButton Sender)
{
	...
	if (Button.ActionID == 801) { Cats = class'WOTCTrainerItems'.static.Categories(); RefreshCatalogue(); return; }
...
simulated function ConfirmGrant()
{
	...
	PendingOld = Owned;
	PendingNew = NewTotal;
	ShowConfirmation(class'WOTCTrainerItems'.static.Label(class'WOTCTrainerItems'.static.Find(SelectedName))
		$ ": " $ ChangeText() $ "<br><br>" $ class'WOTCTrainerText'.default.GrantHelp, false);
}
...
simulated function OnConfirmed(name Action)
{
	...
	Success = class'WOTCTrainerItems'.static.Grant(SelectedName, PendingQuantity, NewTotal, ErrorCode);
	if (Success)
